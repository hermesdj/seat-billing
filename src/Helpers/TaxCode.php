...
class TaxCode
{
    ...
    private static function generateTaxCode($type, $id): string
    {
        return sprintf("tax%02X%02X%016X", self::CURRENT_VERSION, $type, $id);
    }

    public static function generateInvoiceCode($invoice_id): string
    {
        return self::generateTaxCode(self::SINGLE_INVOICE_CODE, $invoice_id);
    }

    public static function generateCorporationCode($corporation_id): string
    {
        return self::generateTaxCode(self::CORPORATION_INVOICE_CODE, $corporation_id);
    }

    public static function decodeTaxCode($code): ?TaxCode
    {
        $code = trim($code);

        if(!str_starts_with($code, "tax")) return null;
    ...
    private static function decodeV1($code): ?TaxCode
    {
